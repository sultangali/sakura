...
### 3.1. Установка инструментов для конвертации WMF/EMF формул (опционально)

**Важно:** Система работает и БЕЗ установки дополнительных инструментов! Если ImageMagick не установлен, WMF/EMF файлы будут заменены на PNG-заглушки с текстом.

Для корректного отображения математических формул из Word рекомендуется установить ImageMagick:

#### Windows:
- Скачайте: https://imagemagick.org/script/download.php
- Установите с опцией "Install legacy utilities (e.g. convert)"
...
**Что происходит без ImageMagick:**
- WMF/EMF файлы конвертируются в PNG с текстом-заглушкой
- Обычные изображения (PNG/JPG) работают нормально
...
- `POST /api/questions/upload` - Загрузить файл (form-data: file, subjectId)
...
Система поддерживает загрузку вопросов из следующих форматов:

### 1. DOCX (рекомендуется)
- Полная поддержка изображений и формул
- Автоматическое извлечение изображений
- Конвертация WMF/EMF формул в PNG

### 2. DOC (старый формат Word)
- Требуется LibreOffice, antiword или catdoc для конвертации
- Конвертируется в DOCX или TXT перед парсингом
- Изображения поддерживаются при конвертации через LibreOffice

### 3. TXT (простой текст)
- Работает без дополнительных программ
- Только текст, без изображений
- Самый быстрый формат для простых вопросов

## Формат файла с вопросами

Все форматы используют одинаковую структуру:
...
- Изображения автоматически извлекаются из DOCX файлов
- Формулы конвертируются в PNG для отображения в браузере
